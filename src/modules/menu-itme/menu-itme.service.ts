...
export class MenuItmeService {
  ...
  getOne(id: string) {
    let menuItem = this.menuItemModel.findById(id);
    if (!menuItem) return new NotFoundException('the menu item dosent exsist');
    return menuItem;
  }

  update(id: string, menuItem: any) {
    let exsintngmenuItem = this.menuItemModel.findOneAndUpdate(
      { id },
      menuItem,
    );
    if (!menuItem) return new NotFoundException('the menu item dosent exsist');
  ...
  delete(id: string) {
    let menuItem = this.menuItemModel.findByIdAndDelete(id);
    return menuItem;
  }
}
